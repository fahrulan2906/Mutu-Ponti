...
export async function analyzeLevelData(level: 'PAUD' | 'SD' | 'SMP', content: string, year: string) {
  ...
  try {
    ...
    const prompt = `ANDA: Pakar Analisis Rapor Pendidikan Indonesia.
      ...
      3. PRIORITAS ${level}:
         - Jika SMP/SD (Dasmen), wajib cari data untuk 5 Indikator: 1. Kemampuan Literasi, 2. Kemampuan Numerasi, 3. Karakter, 4. Iklim Keamanan, 5. Iklim Kebhinekaan.
         - Jika PAUD, wajib cari data untuk 4 Indikator: 1. Perencanaan untuk Proses Pembelajaran yang efektif, 2. Proses belajar yang sesuai bagi anak usia dini, 3. Pembelajaran yang membangun kemampuan fondasi, 4. Indeks ketersediaan Sarana Prasarana Esensial.
      4. DASHBOARD: Hitung rata-rata dimensi (0-100). Jika data 0, rata-rata dimensi tersebut harus 0.
      5. NILAI PRIORITAS: Pastikan 'priorities' di JSON berisi tepat item-item di atas sesuai jenjangnya. Gunakan nama indikator yang persis sama seperti instruksi nomor 3.
